...
int makeKDTree(int l, int r, int depth){
    ...
    if( depth % 2 == 0){
        sort(P + l, P + r, lessX);
//        cout << endl << "lessX = " << lessX << endl;
    }else{
        sort(P + l, P + r, lessY);
//        cout << endl << "lessY = " << lessY << endl;
//http://blog.cafeform.com/?p=1453
//第三引数で、比較条件を読み込んでいる。ここでは　p1.x < p2.x　となるようにソートするという条件
//ソート対象が、独自クラスのPointなので、ソート基準を明記しなければ、ソートできない
    }
    T[t].location = mid;
